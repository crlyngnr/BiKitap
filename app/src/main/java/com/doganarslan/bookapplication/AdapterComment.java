...
import com.doganarslan.bookapplication.databinding.RowCommentBinding;
...
public class AdapterComment extends RecyclerView.Adapter<AdapterComment.HolderComment> {
    ...
    private void deleteComment(ModelComment modelComment, HolderComment holder) {
        ...
        builder.setTitle("Yorumu sil")
                .setMessage("Bu yorumu silmek istediğinizden emin misiniz?")
                .setPositiveButton("Sil", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                    DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Books");
                    ref.child(modelComment.getBookId()).child("Comments").child(modelComment.getTimestamp())
                            .removeValue()
                            .addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onFailure(@NonNull Exception e) {
                                    Toast.makeText(context, "Silme işlemi sırasında hata oluştu: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                                }
                            });
                    }
